	...
	{	
		...
		_actionText = format ["<t color='#FFC0CB'>Spawn %1", _displayName];
				...
				[_obj, 
					...
					[_actionText,   
						...
						{ 
							...
							[[_caller, _type], {
								...
								private ["_type","_pos", "_dir", "_veh", "_marker", "_caller", "_turret", "_turretType", "_posRel"];
					...
					, _x, 99, true, true, "", "(_this distance _target < 3) and (typeOf _this == 'B_Soldier_UAV_F') and (count (activeUGVs select {alive _x}) < 1) and (alive _target) and ((typeOf _target == 'Land_PCSet_01_screen_F') or (count (_target nearEntities [['Man'], 5] select {typeOf _x == 'B_recon_JTAC_F'}) > 0))" ] 
