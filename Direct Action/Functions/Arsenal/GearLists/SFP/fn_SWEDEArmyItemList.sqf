

		_arsenalHeadgear = [
			
			"sfp_helmets_base",
			"sfp_m90w_helmet",
			"sfp_m90w_helmet_peltor",
			"sfp_m90w_helmet_headset",
			"sfp_m90w_helmet_nvg",
			"sfp_m90w_helmet_peltor_nvg",
			"sfp_m90w_helmet_headset_nvg",
			"sfp_m90w_cap",
			"sfp_m90w_cap_off",
			"sfp_m90w_cap_plt",
			"sfp_m90w_cap_grp",
			"sfp_m90w_cap_alt",
			"sfp_m90w_cap_alt_off",
			"sfp_m90w_cap_alt_plt",
			"sfp_m90w_cap_alt_grp", 
			"sfp_telehelmet_2",
			"sfp_flighthelmet116",
			"sfp_wool_beanie_green",
			"sfp_caps_base",
			"sfp_batmossa_m48",
			"sfp_bashatt90",
			"sfp_m90w_cap_peltor",
			"sfp_m90w_booniehat",
			"sfp_m87_flying_suit",
			
			"FIR_ScorpionHMCS"

		];
		
		
		_arsenalGoggles = [

			"G_AirPurifyingRespirator_01_F",
			"G_Balaclava_blk",
			"G_Balaclava_combat",
			"G_Balaclava_lowprofile",
			"G_Balaclava_oli",
			"G_Balaclava_TI_blk_F",
			"G_Balaclava_TI_G_blk_F",
			"G_Balaclava_TI_G_tna_F",
			"G_Balaclava_TI_tna_F",
			"G_Combat",
			"G_Combat_Goggles_tna_F",
			"G_B_Diving",
			"G_Lowprofile",
			"G_Tactical_Black",
			"G_Tactical_Clear"
			
		];
		
		
		_arsenalUniforms = [
			
			"sfp_m90w_uniform",
			"sfp_m90w_uniform_dirty",
			"sfp_m90w_uniform_modern",
			"sfp_m90w_uniform_mp",
			"sfp_m90w_uniform_m08",
			"sfp_m90w_uniform_boots_m08",
			"sfp_m90w_uniform_ksk90",
			"sfp_m90w_uniform_ksk90_modern",
			"sfp_m90w_uniform_tshirt",
			"sfp_m90w_uniform_folded",
			"sfp_m90w_uniform_long_m08",
			"sfp_m90w_uniform_fs18",
			"sfp_m90w_uniform_tucked_m08"

		];

		
		_arsenalVests = [

			"sfp_rebreather_old",
			"sfp_kroppsskydd12",
			"sfp_kroppsskydd12_crew",
			"sfp_kroppsskydd12_tl",
			"sfp_kroppsskydd12_gl",
			"sfp_kroppsskydd12_mg",
			"sfp_kroppsskydd12_at",
			"sfp_kroppsskydd12_medic",
			"sfp_kroppsskydd12_holster",
			"sfp_stridssele_belt"

		];
		
		

		_arsenalBackpacks = [
					
			"sfp_stridssack2000_ra_ksk90",
			"sfp_stridssack2000_ra_ksk12",
			"sfp_lk35_ra145",
			"sfp_ra145",
			"ffp_ito2005m_support_bag",
			"ffp_ito2005m_weapon_bag",
			"sfp_rbs55_support_bag",
			"ffp_orbiter_bag",
			"ffp_orbiter_ramp_bag",
			"sfp_bag_base",
			"sfp_dummy_bag",
			"sfp_backpack_lk35",
			"sfp_backpack_lk35_assault",
			"sfp_backpack_lk35_ksp58",
			"sfp_backpack_lk35_ksp90",
			"sfp_backpack_lk35_explosives",
			"sfp_backpack_lk35_mines",
			"sfp_backpack_lk35_repair",
			"sfp_backpack_rb56_loader",
			"sfp_backpack_grg_loader",
			"sfp_backpack_grg_loader_smoke",
			"sfp_backpack_grg_loader_flare",
			"sfp_backpack_sjvv9",
			"sfp_backpack_stridssack08",
			"sfp_backpack_stridssack08_assault",
			"sfp_backpack_stridssack08_ksp58",
			"sfp_backpack_stridssack08_ksp90",
			"sfp_backpack_stridssack08_explosives",
			"sfp_backpack_stridssack08_repair",
			"sfp_backpack_stridssack2000",
			"sfp_backpack_stridssack2000_assault",
			"sfp_backpack_stridssack2000_ksp58",
			"sfp_backpack_stridssack2000_ksp90",
			"sfp_backpack_stridssack2000_explosives",
			"sfp_backpack_stridssack2000_repair",
			"sfp_stridssele_backpack",
			"sfp_stridssele_backpack_assault",
			"sfp_stridssele_backpack_grg",
			"sfp_stridssele_backpack_ksp58",
			"sfp_stridssele_backpack_explosive",
			"sfp_grkm41_support_bag",
			"sfp_grkm41_weapon_bag",
			"sfp_grkm84_support_bag",
			"sfp_grkm84_weapon_bag",
			"sfp_grsp_support_bag",
			"sfp_grsp_weapon_bag",
			"sfp_rbs17_support_bag",
			"sfp_rbs17_weapon_bag",
			"sfp_rbs17_missile_bag",
			"sfp_rbs55_weapon_bag",
			"sfp_rbs56_support_bag",
			"sfp_rbs56_weapon_bag",
			"sfp_rbs70_support_bag",
			"sfp_rbs70_weapon_bag",
			"sfp_ksp88_support_bag",
			"sfp_ksp88_weapon_bag",
			"sfp_tent12_bag",
			"sfp_kamin_bag"
			
		];


		_arsenalNvgoggles = [


		];


		_arsenalPrimaryAttachments = [

			"rhsusf_acc_harris_bipod",
			"rhsusf_acc_harris_swivel",
			"rhsusf_acc_saw_bipod",
			"rhsusf_acc_saw_lw_bipod",
			"rhsusf_acc_m14_bipod",
			"rhsusf_acc_grip1",
			"rhsusf_acc_grip2",
			"rhsusf_acc_grip3",
			"rhsusf_acc_grip4",
			"rhsusf_acc_grip4_bipod",
			"rhsusf_acc_rvg_blk",
			"rhsusf_acc_tacsac_blk",
			"rhsusf_acc_tdstubby_blk",
			"rhsusf_acc_kac_grip",
			"rhsusf_acc_kac_grip_saw_bipod",
			"rhs_acc_at4_handler",
			"rhsusf_acc_anpeq15A",
			"rhsusf_acc_anpeq15_bk",
			"rhsusf_acc_anpeq15_h",
			"rhsusf_acc_anpeq15_sc",
			"rhsusf_acc_anpeq15_bk_h",
			"rhsusf_acc_anpeq15_bk_sc",
			"rhsusf_acc_anpeq15_bk_light",
			"rhsusf_acc_anpeq15_light_h",
			"rhsusf_acc_anpeq15_light_sc",
			"rhsusf_acc_anpeq15_bk_light_h",
			"rhsusf_acc_anpeq15_bk_light_sc",
			"rhsusf_acc_anpeq15_wmx_h",
			"rhsusf_acc_anpeq15_wmx_light_h",
			"rhsusf_acc_anpeq15_wmx_sc",
			"rhsusf_acc_anpeq15_wmx_light_sc",
			"rhsusf_acc_anpeq15_bk_top",
			"rhsusf_acc_anpeq15_top_h",
			"rhsusf_acc_anpeq15_bk_top_h",
			"rhsusf_acc_anpeq15_top_sc",
			"rhsusf_acc_anpeq15_bk_top_sc",
			"rhsusf_acc_anpeq15side_bk",
			"rhsusf_acc_anpeq16a",
			"rhsusf_acc_anpeq16a_light",
			"rhsusf_acc_anpeq16a_top",
			"rhsusf_acc_anpeq16a_light_top",
			"rhsusf_acc_anpeq16a_top_h",
			"rhsusf_acc_anpeq16a_light_top_h",
			"rhsusf_acc_anpeq16a_top_sc",
			"rhsusf_acc_anpeq16a_light_top_sc",
			"rhsusf_acc_M952V",			
			"rhsusf_acc_wmx_bk",
			"rhsusf_acc_M2010S",
			"rhsusf_acc_M2010S_sa",
			"rhsusf_acc_SR25S",
			"rhsusf_acc_rotex5_grey",
			"rhsusf_acc_nt4_black",
			"rhsusf_acc_omega9k",
			"rhsusf_acc_rotex_mp7",
			"rhsusf_acc_muzzleFlash_ar15",
			"rhsusf_acc_muzzleFlash_SF3P556",
			"rhsusf_acc_muzzleFlash_SFMB556",
			"rhsusf_acc_muzzleFlash_ardec_m240",
			"rhsusf_acc_muzzleFlash_M14_USGI_NM",
			"rhsusf_m4muzzlebreak_base",
			"rhsusf_acc_SF3P556",
			"rhsusf_acc_SFMB556",
			"rhsusf_acc_ARDEC_M240",
			"rhsusf_acc_m14_flashsuppresor",
			"rhsusf_acc_m24_silencer_black",
			"rhsusf_acc_m24_muzzlehider_black",
			"rhsusf_acc_aac_scarh_silencer",
			"rhsusf_acc_aac_762sd_silencer",
			"rhsusf_acc_aac_762sdn6_silencer",
			"rhsusf_acc_aac_m14dcqd_silencer",
			"rhsusf_acc_sniper_base",
			"rhsusf_acc_compm4",
			"rhsusf_acc_T1_high",
			"rhsusf_acc_T1_low",
			"rhsusf_acc_T1_low_fwd",
			"rhsusf_acc_T1_low_fwd_ak",
			"rhsusf_acc_RX01",
			"rhsusf_acc_RX01_NoFilter",
			"rhsusf_acc_RM05",
			"rhsusf_acc_RM05_fwd",
			"rhsusf_acc_RM05_fwd_ak",
			"rhsusf_acc_mrds",
			"rhsusf_acc_mrds_fwd",
			"rhsusf_acc_mrds_fwd_ak",
			"rhsusf_acc_eotech_552",
			"rhsusf_acc_anpvs27",
			"rhsusf_acc_anpas13gv1",
			"rhsusf_acc_LEUPOLDMK4",
			"rhsusf_acc_ELCAN",
			"rhsusf_acc_ELCAN_ard",
			"rhsusf_acc_elcan_3d",
			"rhsusf_acc_elcan_ard_3d",
			"rhsusf_acc_ELCAN_pip",
			"rhsusf_acc_ELCAN_ard_pip",
			"rhsusf_acc_su230_base",
			"rhsusf_acc_su230_base_3d",
			"rhsusf_acc_su230_mrds_base",
			"rhsusf_acc_su230_mrds_base_3d",
			"rhsusf_acc_su230",
			"rhsusf_acc_su230_3d",
			"rhsusf_acc_su230_mrds",
			"rhsusf_acc_su230_mrds_3d",
			"rhsusf_acc_su230a",
			"rhsusf_acc_su230a_3d",
			"rhsusf_acc_su230a_mrds",
			"rhsusf_acc_su230a_mrds_3d",
			"rhsusf_acc_SpecterDR",
			"rhsusf_acc_SpecterDR_D",
			"rhsusf_acc_SpecterDR_OD",
			"rhsusf_acc_SpecterDR_CX",
			"rhsusf_acc_SpecterDR_A",
			"rhsusf_acc_SpecterDR_3d",
			"rhsusf_acc_SpecterDR_D_3D",
			"rhsusf_acc_SpecterDR_OD_3D",
			"rhsusf_acc_SpecterDR_A_3d",
			"rhsusf_acc_SpecterDR_CX_3D",
			"rhsusf_acc_SpecterDR_pvs27",
			"rhsusf_acc_ACOG",
			"rhsusf_acc_ACOG_sa",
			"rhsusf_acc_ACOG2",
			"rhsusf_acc_ACOG3",
			"rhsusf_acc_ACOG_USMC",
			"rhsusf_acc_ACOG2_USMC",
			"rhsusf_acc_ACOG3_USMC",
			"rhsusf_acc_ACOG_RMR",
			"rhsusf_acc_ACOG_3d",
			"rhsusf_acc_ACOG_sa_3d",
			"rhsusf_acc_ACOG2_3d",
			"rhsusf_acc_ACOG3_3d",
			"rhsusf_acc_ACOG_USMC_3d",
			"rhsusf_acc_ACOG2_USMC_3d",
			"rhsusf_acc_ACOG3_USMC_3d",
			"rhsusf_acc_ACOG_RMR_3d",
			"rhsusf_acc_ACOG_pip",
			"rhsusf_acc_ACOG_sa_pip",
			"rhsusf_acc_ACOG2_pip",
			"rhsusf_acc_ACOG3_pip",
			"rhsusf_acc_ACOG_USMC_pip",
			"rhsusf_acc_ACOG2_USMC_pip",
			"rhsusf_acc_ACOG3_USMC_pip",
			"rhsusf_acc_ACOG_RMR_pip",
			"rhsusf_acc_ACOG_anpvs27",
			"rhsusf_acc_LEUPOLDMK4_2",
			"rhsusf_acc_LEUPOLDMK4_2_mrds",
			"rhsusf_acc_premier",
			"rhsusf_acc_premier_mrds",
			"rhsusf_acc_premier_low",
			"rhsusf_acc_premier_anpvs27",
			"rhsusf_acc_M8541",
			"rhsusf_acc_M8541_mrds",
			"rhsusf_acc_M8541_low",
			"rhsusf_acc_M2A1",
			"rhsusf_acc_eotech_xps3",
			"rhsusf_acc_g33_xps3",
			"rhsusf_acc_g33_xps3_flip",
			"rhsusf_acc_g33_T1",
			"rhsusf_acc_g33_T1_flip",
			"rhs_weap_optic_smaw",
			"rhs_weap_optic_smaw_1",
			"rhs_weap_optic_smaw_2",
			"rhs_weap_optic_smaw_3",
			"rhs_weap_optic_smaw_4",
			"rhs_weap_optic_smaw_5",
			"rhs_weap_optic_smaw_6",
			"rhs_weap_optic_smaw_7",
			"rhs_weap_optic_smaw_8",
			"rhs_weap_optic_smaw_9",
			"rhs_weap_optic_smaw_10",
			
			"acc_flashlight",
			"acc_pointer_IR",
			
			"sfp_optic_aimpoint",
			"sfp_optic_aimpoint_t1",
			"sfp_optic_3x_aimpoint",
			"sfp_optic_aimpoint_twist",
			"sfp_optic_hensoldt_4x",
			"sfp_optic_kikarsikte09_4x",
			"sfp_optic_fcs13",
			"sfp_optic_grg",
			"sfp_optic_gcgm",
			"sfp_optic_susat_4x",
			"sfp_optic_kikarsikte90b_10x",
			"sfp_dbal2",
			"sfp_tlr2",
			"sfp_ra180"
		
		];


		_arsenalSecondaryAttachments = [

		];
		
		_arsenalExplosives = [
		
			"SatchelCharge_Remote_Mag",
			"DemoCharge_Remote_Mag",
			"SLAMDirectionalMine_Wire_Mag",
			"ClaymoreDirectionalMine_Remote_Mag",
			"HandGrenade",
			"MiniGrenade",
			"SmokeShell",
			"SmokeShellRed",
			"SmokeShellGreen",
			"SmokeShellYellow",
			"SmokeShellPurple",
			"SmokeShellBlue",
			"SmokeShellOrange",
			"Chemlight_green",
			"Chemlight_red",
			"Chemlight_yellow",
			"Chemlight_blue",
			"B_IR_Grenade",
			"Laserbatteries"
		
		];


		_arsenalItems = [
		
			"rhsusf_bino_lerca_1200_black",
			"rhsusf_bino_lerca_1200_tan",
			"lerca_1200_black",
			"lerca_1200_tan",
			"rhsusf_bino_lrf_Vector21",
			"rhsusf_lrf_Vector21",
			"rhsusf_bino_leopold_mk4",
			"Leupold_Mk4",
			"rhsusf_bino_m24",
			"rhsusf_bino_m24_ARD",
			"m24_bino",
			"m24_bino_ARD",
			
			"ItemGPS",
			"B_UavTerminal",
			"Laserdesignator",
			"MineDetector",
			"Binocular",
			"FirstAidKit",
			"ItemCompass",
			"Rangefinder",
			"ItemWatch",
			"ToolKit",
			"ItemRadio",
			"Medikit",
			"ItemMap"

		];
				
		_return = [_arsenalHeadgear, _arsenalGoggles, _arsenalUniforms, _arsenalVests, _arsenalBackpacks, _arsenalNvgoggles, _arsenalPrimaryAttachments, _arsenalSecondaryAttachments, _arsenalExplosives, _arsenalItems];
		
		_return
